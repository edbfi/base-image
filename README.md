...
Shared Renovate defaults use `edbfi/automation` at `v4.0.0`, with GitHub auto-merge behind required CI. Run local validation with `python3 -m unittest discover -s tools -p 'test_*.py'`, `actionlint`, and `shellcheck tools/smoke.sh`.
